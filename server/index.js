...
app.post("/api/users", (req, res) => {
    ...
    db.query(
        sqlInsert,
        [email, hashedPassword.hash, hashedPassword.salt, name],
        (err, result) => {
            if (err) {
...
app.post("/api/courses", (req, res) => {
    ...
    const sqlInsert =
        "INSERT INTO courses (CourseCode, CourseName, TrimesterTaught) VALUES (?, ?, ?)";
...
app.post("/api/assignments", (req, res) => {
    ...
    db.query(
        sqlInsert,
        [courseCode, trimester, assignmentName, weight, dueDate],
        (err, result) => {
            reply(res, err, result);
        }
    );
...
app.post("/api/user/courses", (req, res) => {
    ...
    db.query(
        sqlInsert,
        [userID, courseCode, year, trimester, grades, totalGrade],
        (err, result) => {
            reply(res, err, result);
        }
    );
...
app.get("/api/:course/assignments", (req, res) => {
    ...
    const sqlSelect =
        "SELECT * FROM assignments WHERE CourseCode = ? AND Trimester = ?";
...
app.put("/api/user", (req, res) => {
    ...
    const sqlUpdate =
        "UPDATE users SET Email = ?, Password = ?, DisplayName = ? WHERE Email = ?";
...
app.put("/api/courses/", (req, res) => {
    ...
    const sqlUpdate =
        "UPDATE grades SET Grades = ? WHERE Email = ? AND CourseCode = ? AND Year = ? AND Trimester = ?";
    db.query(
        sqlUpdate,
        [grades, userId, courseCode, year, trimester],
        (err, result) => {
            reply(res, err, result);
        }
    );
