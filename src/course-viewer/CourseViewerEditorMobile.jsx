    ...
    return (
        <Dialog open={currentEdit !== null && isMobile} onClose={() => setCurrentEdit(null)}>
            <Stack sx={{ display:"flex", alignItems:"center", mx: 3, my: 2 }}>
                ...
                <TextField label="Worth (%)" type="number" InputProps={{ inputProps: { min: 0 } }} value={ currentEdit ? currentEdit.weight : "0" } 
                    onChange={(e) => {
                        currentEdit.setWeight(e.target.value);
                        checkChanges();
                    }} 
                    error={currentEdit && (currentEdit.weight <= 0 || currentEdit.weight > 100)} sx={{ mt: 2, width: "90%" }}
